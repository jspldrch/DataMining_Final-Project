"""
run_v7.py  –  Drought Severity Prediction v7

Base: run_v4 (MAE 0.8587) – only safe, proven additions.

v6 regressed to 1.1260 because:
  - regression_l1 objective destabilises LGB training (L1 gradient = ±1, no magnitude)
  - Optuna tuned only on week-1 proxy → bad generalisation to weeks 2-5
  - z-score merge could introduce NaNs for unseen (region, month) pairs

v7 fixes / additions (run_v4 structure kept intact):
  1. humidity added to LAG_COLS + ROLL_COLS   — strong drought signal, low risk
  2. 180-day rolling window                    — multi-month drought memory
  3. week_of_year sin/cos                      — finer seasonality than month alone
  4. regional_mean_score                       — per-region avg drought level (train only)
  5. num_leaves 63 → 127                       — more capacity, same regularisation
  6. LGB objective stays "regression" (L2)     — what works in v4, not changed

Usage:
    python scripts/run_v7.py
Output: outputs/submission_v7.csv
"""
...
OUT_PATH   = OUT_DIR / "submission_v7.csv"

# ─── Mode ─────────────────────────────────────────────────────────────────────
QUICK_MODE = False   # True = ~20 min;  False = full Kaggle run (~90 min)
...
# humidity added vs v4 – strong short-term drought signal
LAG_COLS  = ["tmp_range", "tmp_max", "tmp", "prec", "wind", "surf_pre", "humidity"]
LAGS      = [1, 3, 7, 14, 21]
# humidity added vs v4;  180d window added for multi-month drought memory
ROLL_COLS = ["prec", "wind", "tmp", "humidity"]
ROLL_WINS = [7, 14, 30, 60, 90, 180]

LGB_PARAMS = dict(
    objective="regression",       # L2/MSE — same as v4, do not change (v6 proved L1 is worse here)
    metric="mae",
    n_estimators=N_ESTIMATORS,
    learning_rate=0.04,
    num_leaves=127,               # 63 → 127: more capacity, same regularisation
...
def build_feature_list() -> list[str]:
    ...
    # week_sin/week_cos: finer seasonality than month (52 steps vs 12)
    ...
    # regional_mean_score: how drought-prone is this region on average?
...
def compute_regional_mean_score(train_raw: pd.DataFrame) -> pd.Series:
    """Average drought score per region from training data. Captures drought-prone regions."""
...
def compute_region_features(
    ...
    All new columns collected in a dict and assigned via pd.concat.
    """
    te = te.copy()
    te["score"] = np.nan
    ...
    # Week-of-year: finer seasonality signal (52 positions vs 12 for month)
    ...
    for col in ROLL_COLS:
        ...
        for w in ROLL_WINS:
            min_p = max(3, w // 10)   # 180d needs min_periods=18 to avoid all-NaN
...
def main() -> None:
    ...
    t0 = time.time()
    print("=" * 66)
    print("  Natural Disaster Severity Prediction  -  run_v7.py")
    ...
    print(f"  New vs v4: +humidity lags/rolls, +180d window, +week_sin/cos, +regional_mean_score")
    print("=" * 66)
    ...
    # Compute regional mean score from training data (no leakage)
    ...
    # Add regional mean score (computed before del train_raw)
