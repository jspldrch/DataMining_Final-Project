"""
kaggle_v2_seasonal.py  –  Baseline + Saisonale Abweichung

EINZIGE ÄNDERUNG vs v1_base:
  + prec_seasonal_dev, humidity_seasonal_dev, tmp_seasonal_dev
  (Wert minus historischer Region-Monats-Mittelwert)

Alles andere (Modell, Ensemble, X_test, Caches) identisch zu v1.
Cache: nutzt GLEICHEN weekly cache wie v1 (gleiche Basis-Features)
"""
...
WEEKLY_CACHE  = WORK_DIR / "cache_weekly.npz"        # GETEILT mit v1/v3
WINDOWS_CACHE = WORK_DIR / "cache_windows_v2.npz"    # nur v2
OUT_PATH      = WORK_DIR / "submission_v2_seasonal.csv"
...
# ═══ ÄNDERUNG vs v1: seasonal_dev berechnen ═══
def add_extra_features(weekly: pd.DataFrame) -> pd.DataFrame:
    """Saisonale Abweichung: Wert minus historischer Region-Monats-Mittelwert."""
    weekly = weekly.sort_values(["region_id", "ordinal"]).copy()
    # Monat aus ordinal (Fix: Monat 12 → 12*31=372 → 0 nach % → korrigiert zu 12)
...
def print_importance(lgb_models,top=20):
    ...
    # Seasonal Dev-Anteil hervorheben
    sdev_mask = np.array(["seasonal_dev" in n for n in feat])
    if sdev_mask.any():
        print(f"  → seasonal_dev gesamt: {100*imp[sdev_mask].sum()/total:.2f}%")
    print(f"{'─'*55}\n")
...
def main():
    ...
    print("="*60)
    print(f"  kaggle_v2_seasonal  |  Features: {len(FEATURES)}")
    print(f"  ÄNDERUNG vs v1: +prec/humidity/tmp_seasonal_dev")
    print("="*60)
    ...
    # ══ ÄNDERUNG: seasonal_dev hinzufügen ══
    ...
    # ══ ÄNDERUNG: seasonal_dev für Test berechnen (letzter bekannter Wert) ══
