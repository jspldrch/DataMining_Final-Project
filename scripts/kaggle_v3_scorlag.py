"""
kaggle_v3_scorlag.py  –  Baseline + score_lag mit KORREKTEM X_test

EINZIGE ÄNDERUNG vs v1_base:
  + score_lag1/2/3 als Features
  + X_test kommt vom LETZTEN TRAININGS-TAG (nicht letztem Test-Tag)

Warum das wichtig ist:
  v15/v17 (lokal): X_test = letzter Test-Tag
    → score_lag1 = Score von 13 Wochen VOR dem Prediction-Zeitpunkt (veraltet)
    → Modell verlässt sich 78-91% auf veralte Info → Kaggle-Score 1.04-1.08

  v3 (dieser Script): X_test = letzter Trainings-Tag
    → score_lag1 = aktuellster bekannter Score (direkt vor Prediction) (frisch!)
    → Trainings-Szenario und Test-Szenario identisch ausgerichtet
    → Prediction = Wochen 1-5 nach dem letzten Trainings-Datum

Cache: nutzt GLEICHEN weekly cache wie v1/v2 (gleiche Basis-Features)
"""
...
WEEKLY_CACHE  = WORK_DIR / "cache_weekly.npz"        # GETEILT mit v1/v2
WINDOWS_CACHE = WORK_DIR / "cache_windows_v3.npz"    # nur v3
OUT_PATH      = WORK_DIR / "submission_v3_scorlag.csv"
...
def load_npz(path: Path) -> pd.DataFrame:
    d=np.load(path,allow_pickle=True); names=d["region_names"]
    df=pd.DataFrame({"region_id":names[d["region_id"]],"year":d["year"].astype(np.int32),
                     "month":d["month"].astype(np.int32),"day":d["day"].astype(np.int32)})
...
def _region_features(tr,te):
    ...
    for col in LAG_COLS:
        for lag in LAGS: nc[f"{col}_lag{lag}"]=panel[col].shift(lag).astype(np.float32)
    ...
    n=len(tr); return panel.iloc[:n].copy(), panel.iloc[n:].copy()
...
def load_weekly(t0):
    if WEEKLY_CACHE.exists():
        ...
        # v3 braucht auch den letzten Trainings-Tag als X_test-Basis
        # X_test_base aus Cache ist der letzte TEST-Tag — für v3 ignorieren wir den
    ...
    train_path = _find_npz("train.npz"); test_path = _find_npz("test.npz")
    print(f"  train: {train_path}"); print(f"  test:  {test_path}")
    train_raw=load_npz(train_path); test_raw=load_npz(test_path)
    ...
    # Speichere letzten Test-Tag (für v1/v2 kompatiblen Cache)
    ...
    np.savez_compressed(WEEKLY_CACHE,
        ...
        X_test_base=X_test_arr,test_region_ids=test_region_ids,
...
# ══ ÄNDERUNG vs v1: score_lag aus wöchentlichen Trainingsdaten berechnen ══
def add_extra_features(weekly: pd.DataFrame) -> pd.DataFrame:
    """Score-Lags: lag1 = aktueller Score, lag2/3 = Vorwochen."""
...
def _build_windows(weekly,skip,features,stride=1):
    ...
    X["region_id"]=pd.Categorical(rp); return X,np.vstack(yp).astype(np.float32)
...
def load_or_build_windows(weekly,val_regions,features,t0):
    ...
    print(f"  Cache gespeichert [{elapsed(t0)}]"); return X_tr,y_tr,X_va,y_va,X_all,y_all
...
def train_lgb(X_tr,y_tr,X_va,y_va,n_trees=None):
    ...
    for wk in range(5):
        ...
        m.fit(X_tr,y_tr[:,wk].ravel(),**kw); models.append(m)
    return models

def train_xgb(X_tr,y_tr,X_va,y_va,features,n_trees=None):
    Xn=X_tr[features].to_numpy(np.float32); Vn=X_va[features].to_numpy(np.float32) if X_va is not None else None
    models=[]
    for wk in range(5):
        n=(n_trees[wk] if n_trees else None) or XGB_P["n_estimators"]
        p=dict(XGB_P,random_state=RANDOM_STATE+wk,n_estimators=n); kw={}
        if Vn is not None: p["early_stopping_rounds"]=50; kw.update(eval_set=[(Vn,y_va[:,wk].ravel())],verbose=False)
        m=xgb.XGBRegressor(**p); m.fit(Xn,y_tr[:,wk].ravel(),**kw); models.append(m)
    return models
...
def train_cat(X_tr,y_tr,X_va,y_va,features,n_trees=None):
    ...
    for wk in range(5):
        ...
        p=dict(CAT_P,iterations=n,random_seed=RANDOM_STATE+wk); kw={}
        if Vn is not None: kw.update(eval_set=(Vn,y_va[:,wk].ravel()),early_stopping_rounds=50)
        m=CatBoostRegressor(**p); m.fit(Xn,y_tr[:,wk].ravel(),**kw); models.append(m)
    return models
...
def print_importance(lgb_models,top=20):
    ...
    print(f"\n{'─'*55}"); print(f"  FEATURE IMPORTANCE (LGB Gain Ø, Top {top})")
    for rank,i in enumerate(order[:top],1):
        print(f"  {rank:<3d}  {feat[i]:<34}  {100*imp[i]/total:>5.2f}%")
    lag_mask=np.array(["score_lag" in n for n in feat])
    if lag_mask.any():
        print(f"  → score_lag gesamt: {100*imp[lag_mask].sum()/total:.2f}%")
    print(f"{'─'*55}\n")

def make_submission(test_preds,test_region_ids,out_path):
    ...
    elif SAMPLE_SUB is not None: template=pd.read_csv(SAMPLE_SUB)[["region_id"]]
    else: template=sub[["region_id"]]
    out=template.merge(sub,on="region_id",how="left")
    for col in [f"pred_week{k+1}" for k in range(5)]: out[col]=out[col].fillna(0.0)
    out.to_csv(out_path,index=False); print(f"  Submission: {out_path.name}  ({len(out):,} Rows)")
...
def main():
    ...
    print("="*62)
    print(f"  kaggle_v3_scorlag  |  Features: {len(FEATURES)}")
    print(f"  ÄNDERUNG vs v1: +score_lag1/2/3  +  X_test = letzter Trainings-Tag")
    print(f"  Hypothese: score_lag ist frisch → sollte besser als v15/v17 lokal sein")
    print("="*62)

    print(f"\n[1/5] Weekly Features ... [{elapsed(t0)}]")
    weekly,_,test_region_ids_from_cache,base_cols=load_weekly(t0)
    print(f"  {len(weekly):,} Rows, {weekly['region_id'].nunique()} Regionen")

    # ══ ÄNDERUNG: score_lag berechnen ══
    ...
    X_tr,y_tr,X_va,y_va,X_all,y_all=load_or_build_windows(weekly,val_regions,FEATURES,t0)
    print(f"  Train: {len(X_tr):,}  Val: {len(X_va):,}  All: {len(X_all):,}")
    show("score_lag1 Persistence",y_va,np.column_stack([X_va["score_lag1"].to_numpy()]*5))

    print(f"\n[3/5] Training ... [{elapsed(t0)}]")
    lgb_m=train_lgb(X_tr,y_tr,X_va,y_va); lgb_val=pred_lgb(lgb_m,X_va)
    ...
    xgb_m=train_xgb(X_tr,y_tr,X_va,y_va,FEATURES); xgb_val=pred_num(xgb_m,X_va,FEATURES)
    show("XGBoost",y_va,xgb_val)
    preds_val={"lgb":lgb_val,"xgb":xgb_val}
    cat_m=train_cat(X_tr,y_tr,X_va,y_va,FEATURES)
    if cat_m:
        cat_val=pred_num(cat_m,X_va,FEATURES); show("CatBoost",y_va,cat_val); preds_val["cat"]=cat_val
    ...
    print(f"\n[4/5] Final Training ... [{elapsed(t0)}]")
    n_lgb=[_best_n(m,N_ESTIMATORS) for m in lgb_m]; n_xgb=[_best_n(m,N_ESTIMATORS) for m in xgb_m]
    f_lgb=train_lgb(X_all,y_all,None,None,n_lgb)
    f_xgb=train_xgb(X_all,y_all,None,None,FEATURES,n_xgb)
    f_cat=None
    if cat_m:
        n_cat=[_best_n(m,N_ESTIMATORS) for m in cat_m]; f_cat=train_cat(X_all,y_all,None,None,FEATURES,n_cat)
    print(f"  Done [{elapsed(t0)}]")

    # ══ KERN-ÄNDERUNG: X_test = letzter TRAININGS-Tag (nicht letzter Test-Tag) ══
    print(f"\n[5/5] Submission (X_test = letzter Trainings-Tag) ... [{elapsed(t0)}]")
    # weekly enthält die wöchentlichen Trainings-Daten mit score_lag bereits berechnet.
    # Wir nehmen den LETZTEN wöchentlichen Eintrag pro Region als X_test.
    last_train_weekly = (
        weekly.sort_values(["region_id", "ordinal"])
        .groupby("region_id", sort=False)
        .last()
        .reset_index()
    )
    test_region_ids = last_train_weekly["region_id"].values.astype(str)
    X_test = last_train_weekly[FEATURES + ["region_id"]].copy()
    X_test["region_id"] = pd.Categorical(X_test["region_id"])
    # Fehlende Features auf 0 (Fallback)
    for f in FEATURES:
        if f not in X_test.columns: X_test[f] = np.float32(0)

    print(f"  X_test Regionen: {len(X_test):,}")
    print(f"  score_lag1 Mittelwert: {X_test['score_lag1'].mean():.3f}  "
          f"(frisch = letzter bekannter Score pro Region)")

    test_preds=best_w["lgb"]*pred_lgb(f_lgb,X_test)+best_w["xgb"]*pred_num(f_xgb,X_test,FEATURES)
    if f_cat and "cat" in best_w: test_preds+=best_w["cat"]*pred_num(f_cat,X_test,FEATURES)

    # Submission muss alle Regionen aus sample_submission enthalten
    # Eventuell gibt es Regionen im Test die nicht im Training sind → auf 0 setzen
