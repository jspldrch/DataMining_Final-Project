"""
kaggle_v23_short_rolls.py  --  91-Day-Consistent Features + More Trees
=======================================================================
Base:        v19 (last-window val, all regions, no score_lag)
Kaggle best: v22_recent8 = 0.8132

TWO CHANGES vs v19:

  1. ROLL_WINS = [7, 14, 30, 60, 90]   (was [7,14,30,60,90,180])
     -> removes 180d rolling window
     -> features: 121  (was 133, -12 rolling features)
     Why: test set has only 91 days per region.
          roll180 in test = average of 91 days (not 180) -> distribution shift.
          v21a confirmed: longer windows (270/365) hurt Kaggle badly.
          Max usable window = 90d (matches test data length exactly).
     Also: drought indices updated to use max 90d reference (same reason).

  2. N_ESTIMATORS = 2000, LR = 0.02  (was 1000 / 0.04 for LGB + XGB)
     -> v21a showed LGB hitting iteration limit (week 3: iter=1000, never converged)
     -> more trees + lower LR = proper convergence
     -> FINAL_SCALE = 1.1 on final training

Val: last window of ALL regions (~2248 points)  [same as v19]
Notebook 1 datasets (old slugs: trainthis / testset).

Output: /kaggle/working/submission_v23_short_rolls.csv
"""
...
WEEKLY_CACHE  = WORK_DIR / "cache_weekly_v23.npz"
WINDOWS_CACHE = WORK_DIR / "cache_windows_v23.npz"
OUT_PATH      = WORK_DIR / "submission_v23_short_rolls.csv"

def _find_npz(name: str) -> Path:
    for slug in ["trainthis", "testset", "drought-data", "train-data", "datafiles", "data", "input"]:
...
def _find_sample_sub() -> Path | None:
    for p in ["/kaggle/input/sample-submission/sample_submission.csv",
...
DRY_THRESHOLD = 1.0
WINDOW_STRIDE = 1
N_ESTIMATORS  = 2000   # *** CHANGE: was 1000 ***
FINAL_SCALE   = 1.1    # *** CHANGE: scale final training ***
...
# *** CHANGE: removed 180d -- test has only 91 days, roll180 in test = roll91 ***
ROLL_WINS = [7, 14, 30, 60, 90]

LGB_P = dict(objective="regression", metric="mae", n_estimators=N_ESTIMATORS,
             learning_rate=0.02,    # *** CHANGE: was 0.04 ***
             num_leaves=127, min_child_samples=60,
             subsample=0.8, colsample_bytree=0.8, reg_alpha=0.1, reg_lambda=0.1,
             n_jobs=-1, verbose=-1)
XGB_P = dict(objective="reg:squarederror", n_estimators=N_ESTIMATORS,
             learning_rate=0.02,    # *** CHANGE: was 0.04 ***
...
def build_features():
    ...
    # Drought indices -- all reference windows <= 90d (consistent with 91d test limit)
...
def _region_features(tr: pd.DataFrame, te: pd.DataFrame):
    ...
    # *** CHANGE: drought indices now use max 90d reference (was 365d) ***
    # 90d vs 30d deficit: short-term dryness relative to medium-term average
    nc["prec_deficit_90d"] = (pp.rolling(30, min_periods=10).mean() -
                               pp.rolling(90, min_periods=30).mean()).astype(np.float32)
    ...
    nc["humidity_deficit_90d"] = (hp.rolling(30, min_periods=10).mean() -
                                   hp.rolling(90, min_periods=30).mean()).astype(np.float32)

    tp = panel["tmp"].shift(1)
    anom = (tp.rolling(30, min_periods=10).mean() - tp.rolling(90, min_periods=30).mean()).astype(np.float32)
...
def print_importance(lgb_models, features):
    ...
    print(f"    Rolling stats:      {100*r_imp/total:>5.1f}%  (max window: 90d)")
    print(f"    Lags:               {100*l_imp/total:>5.1f}%")
    print(f"    Drought indices:    {100*d_imp/total:>5.1f}%  (max ref: 90d)")
...
def main():
    ...
    print("=" * 60)
    print(f"  kaggle_v23_short_rolls  |  Features: {len(FEATURES)}  |  no score_lag")
    print(f"  CHANGE 1: ROLL_WINS max=90d  (was 180d, v21a had 365d)")
    print(f"  CHANGE 2: LR=0.02, N=2000   (was 0.04 / 1000, v21a hit iter limit)")
    print(f"  Drought indices: now use max 90d reference (was 365d)")
    print(f"  Val: last window of ALL regions (~2248 points)  [same as v19]")
    print(f"  NB1 slugs: trainthis / testset")
    print("=" * 60)
    ...
    for wk in range(5):
        ...
        n_iter = _best_n(lgb_m[wk], N_ESTIMATORS)
        hit_limit = " (HIT LIMIT!)" if n_iter >= N_ESTIMATORS - 5 else ""
        print(f"    Week {wk+1}: iter={n_iter:4d}  MAE={v:.4f}{hit_limit}")
    ...
    # 4. Final training (scaled iterations)
    print(f"\n[4/5] Final training (all windows, scale={FINAL_SCALE}x) ... [{elapsed(t0)}]")
    ...
    print(f"  LGB final iters (×{FINAL_SCALE}): {n_lgb}")
    print(f"  XGB final iters (×{FINAL_SCALE}): {n_xgb}")
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    lgb_iter_avg = sum(_best_n(m, N_ESTIMATORS) for m in lgb_m) / 5
    any_hit_limit = any(_best_n(m, N_ESTIMATORS) >= N_ESTIMATORS - 5 for m in lgb_m)
    print()
    print("=" * 60)
    print(f"  RESULTS -- kaggle_v23_short_rolls")
    print(f"  Change 1: ROLL_WINS=[7,14,30,60,90] max=90d  (v19: max=180d)")
    print(f"  Change 2: LR=0.02, N=2000  (v19: LR=0.04, N=1000)")
    print(f"  {'Features':.<35} {len(FEATURES)}  (v19: 133,  v21a: 157)")
    print(f"  {'Val points':.<35} {len(X_va):,}")
    print(f"  {'Persistence MAE':.<35} {persist_mae:.4f}  (v19: 0.0346)")
    print(f"  {'LightGBM val MAE':.<35} {mae(y_va, lgb_val):.4f}  (v19: 0.2559,  v21a: 0.2239)")
    ...
    print(f"  {'Blend val MAE':.<35} {best_val_mae:.4f}  (v19: 0.2522,  v21a: 0.2272)")
    print(f"  {'Blend weights':.<35} {' '.join(f'{k}={v:.2f}' for k,v in best_w.items())}")
    print(f"  {'LGB avg iters':.<35} {lgb_iter_avg:.0f}  (v19: 457,  v21a: 894)")
    if any_hit_limit:
        print(f"  WARNING: at least one week hit N_ESTIMATORS={N_ESTIMATORS} limit!")
    print(f"  {'-'*56}")
    print(f"  Known Kaggle scores (reference):")
    print(f"  {'  v22_recent8 (best)':.<35} 0.8132")
    print(f"  {'  v19 (last-window val)':.<35} 0.8193")
    print(f"  {'  v12 (baseline)':.<35} 0.8258")
    print(f"  {'  v21a (roll270+365)':.<35} 0.8671  <-- longer windows hurt")
