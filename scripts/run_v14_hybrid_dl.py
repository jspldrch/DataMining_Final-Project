"""
run_v14_hybrid_dl.py  –  Deep Learning Ensemble featuring Hybrid Attention Model
COMPLETE VERSION - Can run locally in Quick Mode or on Kaggle/Colab with GPU.
"""
...
OUT_PATH = OUT_DIR / "submission_v14_hybrid.csv"

# ─── Config ─────────────────────────────────────────────────────────────────────
QUICK_MODE = True  # Set to False for best results (longer training)
...
DL_EPOCHS = 40 if QUICK_MODE else 120
DL_BATCH = 256
DL_LR = 1e-3
DL_HIDDEN = 64
DL_LAYERS = 2
...
def _build_feature_lists() -> None:
    """Build feature lists for GBDT and DL."""
    global GBDT_FEATURES, DL_FEATURES
    
    # GBDT Features (all features)
    ...
    # DL Features (reduced list to prevent overfitting)
...
def _parse_dates_inplace(df: pd.DataFrame) -> None:
    """Fast date parsing without datetime objects."""
...
def compute_region_features(tr: pd.DataFrame, te: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Feature engineering at daily level for a single region."""
    te = te.copy()
    te["score"] = np.nan
...
def daily_to_weekly(df: pd.DataFrame) -> pd.DataFrame:
    """Aggregates daily records to weekly frequency."""
    df = df.copy()
...
def add_score_persist(weekly_df: pd.DataFrame) -> pd.DataFrame:
    """Adds previous week's score as a feature."""
...
def temporal_validation_split(
    ...
    """Generates temporal train/validation split by taking the last weeks of training data."""
    X_train, y_train = [], []
    X_val, y_val = [], []
    val_regions = set()
...
def build_sequence_data(
    ...
    """Builds sliding window sequences for DL models from training data."""
    X_seq, y_seq = [], []
...
def build_val_sequences(
    ...
    """Builds sequences for the validation set."""
    X_seq, y_seq = [], []
...
def build_test_sequences(
    ...
    """Builds sequences for the test set."""
    X_seq, regions = [], []
...
if TORCH_AVAILABLE:
    ...
    # ─── NEU: Hybrid Transformer-MLP Attention Model ────────────────────────────
    class HybridAttentionModel(nn.Module):
        def __init__(
            ...
            d_model: int = 64,
            nhead: int = 4,
            num_encoder_layers: int = DL_LAYERS,
            n_weeks: int = N_WEEKS,
            ...
            # Branch 1: Transformer for the time series
            self.proj = nn.Linear(n_feat, d_model)
            self.pos_emb = nn.Embedding(512, d_model)
            ...
            # Branch 2: Aggregate Features + MLP
            # Aggregates: mean, sum, std, max, min, last step -> 6 summary statistics
            self.num_agg_features = n_feat * 6
            self.mlp = nn.Sequential(
                nn.Linear(self.num_agg_features, mlp_hidden_dim),
            ...
            # Query: MLP projection (1 summary token), Key/Value: Transformer encoder outputs
            ...
            # Final output prediction head
            self.head = nn.Sequential(
                nn.Linear(d_model * 2, 64),
                nn.ReLU(),
                nn.Dropout(dropout),
                nn.Linear(64, n_weeks)
            )
        ...
        def forward(self, x, **kwargs):
            ...
            # 1. Transformer path
            pos = torch.arange(T, device=x.device).unsqueeze(0)
            x_proj = self.proj(x) + self.pos_emb(pos)
            H_trans = self.transformer_encoder(x_proj)  # (B, T, d_model)
            
            # 2. Dynamic statistics calculations over sequence dimension (dim=1)
            x_mean = x.mean(dim=1)  # (B, n_feat)
            x_sum = x.sum(dim=1)    # (B, n_feat)
            x_std = x.std(dim=1)    # (B, n_feat)
            x_max = x.max(dim=1)[0] # (B, n_feat)
            x_min = x.min(dim=1)[0] # (B, n_feat)
            x_last = x[:, -1, :]    # (B, n_feat)
            
            # Concatenate stats
            x_agg = torch.cat([x_mean, x_sum, x_std, x_max, x_min, x_last], dim=1) # (B, 6 * n_feat)
            
            # Pass summary to MLP to obtain query token
            H_mlp = self.mlp(x_agg)  # (B, d_model)
            H_mlp_query = H_mlp.unsqueeze(1)  # (B, 1, d_model)
            
            # 3. Multi-Head Cross-Attention
            # MLP summary queries the Transformer time-series step representations
            ...
            # Concatenate original MLP representation with context-aware sequence vector
            H_fused = torch.cat([H_mlp, attn_out.squeeze(1)], dim=1)  # (B, 2 * d_model)
            
            # Run output head
            out = self.head(H_fused)  # (B, n_weeks)
            return out
    ...
    def train_dl_model(
        ...
        """Trains the PyTorch model with Cosine Annealing learning rate schedule and validation checkpointing."""
    ...
    def predict_dl_model(model: nn.Module, X: np.ndarray, batch_size: int = 1024) -> np.ndarray:
        """Helper for batch prediction using trained PyTorch model."""
        model.eval()
        predictions = []
        model = model.to(DEVICE)
...
def train_gbdt_models(
    ...
    """Trains 5 independent LightGBM regressors (one for each forecasting week)."""
    models = []
...
def main() -> None:
    ...
    print("=" * 70)
    print("  Natural Disaster Severity Prediction  -  run_v14_hybrid_dl.py")
    ...
    if not TRAIN_CSV.exists():
        ...
        print("Please ensure the dataset is downloaded to data/ directory.")
        return

    dtypes = {c: np.float32 for c in WEATHER_COLS}
    train_raw = pd.read_csv(TRAIN_CSV, dtype=dtypes)
    test_raw = pd.read_csv(TEST_CSV, dtype=dtypes) if TEST_CSV.exists() else pd.DataFrame()
    ...
    print("\n[2/6] Feature engineering (daily level calculation)...")
    
    train_by_region = {r: g.reset_index(drop=True) for r, g in train_raw.groupby("region_id", sort=False)}
    test_by_region = {r: g.reset_index(drop=True) for r, g in test_raw.groupby("region_id", sort=False)} if not test_raw.empty else {}
    ...
    test_feat = pd.concat(test_parts, ignore_index=True) if test_parts else pd.DataFrame()
    print(f"   Done | {time.time()-t0:.1f}s")
    
    # ── 3. Weekly aggregation ─────────────────────────────────────────────────
    print("\n[3/6] Weekly aggregation...")
    train_weekly = daily_to_weekly(train_feat[train_feat["score"].notna()])
    test_weekly = daily_to_weekly(test_feat) if not test_feat.empty else pd.DataFrame()
    
    train_weekly = add_score_persist(train_weekly)
    if not test_weekly.empty:
    ...
    X_dl_test, test_regions = build_test_sequences(train_weekly, test_weekly, DL_FEATURES, SEQ_LEN) if not test_weekly.empty else (np.empty(0), [])
    ...
    if TORCH_AVAILABLE and len(X_dl_train) > 0:
        for name, ModelClass in [
            ...
            ("Hybrid-Attention", HybridAttentionModel),
        ]:
            print(f"   {name}...")
            # For HybridAttentionModel we pass seq_len and custom architecture params
            if name == "Hybrid-Attention":
                model = ModelClass(len(DL_FEATURES), seq_len=SEQ_LEN, d_model=DL_HIDDEN)
            else:
    ...
    # ── 7. Final predictions (if test set is available) ──────────────────────
    if not test_weekly.empty:
        ...
        for w, pred in zip(best_weights, all_test_preds):
            final_preds += w * pred
            
        final_preds = np.clip(final_preds, 0, 5)
        
        # Format submission
        sub = pd.DataFrame({"region_id": test_weekly["region_id"].unique()})
        sub = sub.sort_values("region_id").reset_index(drop=True)
        
        for k in range(N_WEEKS):
            sub[f"pred_week{k+1}"] = final_preds[:, k]
            
        sub.to_csv(OUT_PATH, index=False)
        print(f"✅ Submission saved to: {OUT_PATH}")
    else:
        print("\n   [INFO] No test.csv found. Skipping final prediction step.")
