"""
kaggle_v26_recent7.py  --  RECENT_YEARS Sweep: 7 Years
=======================================================
Base:        v22_recent8 (RECENT_YEARS=8, last-window val, 133 features)
Kaggle best: recent_local_8y = 0.8095  |  v22_5y = 0.8132

SINGLE CHANGE vs v22:
  RECENT_YEARS = 7  (was 5 in submitted v22, 8 in v22_recent8 running on NB3)

Goal: complete the RECENT_YEARS sweep with consistent last-window val scheme:
  5y  last-window: 0.8132  (v22, NB3 submitted)
  7y  last-window: ?        <- THIS RUN
  8y  last-window: ?        <- v22_recent8 (NB3, running)
  15y last-window: 0.8193  (v19)

Val: last window of ALL regions (~2248 points)  [identical to v19/v22]
Notebook 4 datasets:
  datafiles/train.npz
  datafiles/test.npz
  samplesub/sample_submission.csv

Output: /kaggle/working/submission_v26_recent7.csv
"""
...
WINDOWS_CACHE = WORK_DIR / "cache_windows_v26.npz"   # new: RY=7 + last-window split
OUT_PATH      = WORK_DIR / "submission_v26_recent7.csv"
...
WINDOW_STRIDE    = 1
N_ESTIMATORS     = 1000
RECENT_YEARS     = 7       # *** CHANGE: sweep point between 5y (0.8132) and 8y (running) ***
...
# ── Feature list (133 features, identical to v19/v22) ────────────────────────
def build_features():
    f = list(WEATHER_COLS)
...
# ── Weekly feature cache (reuse cache_weekly.npz from v19/v22 if present) ────
def load_weekly(t0):
...
# ── Val: last window of ALL regions (identical to v19/v22) ───────────────────
...
def main():
    ...
    print("=" * 60)
    print(f"  kaggle_v26_recent7  |  Features: {len(FEATURES)}  |  no score_lag")
    print(f"  RECENT_YEARS={RECENT_YEARS}  |  Val: last window all regions (2248 pts)")
    print(f"  RECENT_YEARS sweep: 5y=0.8132  7y=?  8y=running  15y=0.8193")
    ...
    print(f"  v22(5y) retained ~33%  |  v19(15y) retained 100%")

    # 3. Sliding windows
    print(f"\n[3/5] Build windows (last-window val) ... [{elapsed(t0)}]")
    ...
    print(f"  v22(5y): Train ~926k  |  v19(15y): Train 1,744k")
    ...
    print(f"  v19 persistence: 0.0346  |  v22(5y) persistence: 0.0346")
    ...
    print(f"  v22(5y) LGB iters: 813/633/608/879/836  (spread=271)")
    print(f"  v19(15y) LGB iters: 477/700/388/363/358  (spread=342)")
    print(f"  -> lower spread = better calibrated early stopping")
    ...
    print(f"  v22(5y): lgb=0.90 xgb=0.05 cat=0.05  MAE=0.2342")
    print(f"  v19(15y): lgb=0.50 xgb=0.05 cat=0.45  MAE=0.2522")
    print_importance(lgb_m, FEATURES)

    # 5. Final training
    print(f"\n[5/5] Final training (all windows) + submission ... [{elapsed(t0)}]")
    ...
    print()
    print("=" * 60)
    print(f"  RESULTS -- kaggle_v26_recent7")
    print(f"  {'RECENT_YEARS':.<40} {RECENT_YEARS}  (47% of ~15y history)")
    print(f"  {'Features':.<40} {len(FEATURES)}  (same as v19/v22)")
    print(f"  {'Val scheme':.<40} last window all 2248 regions")
    print(f"  {'Training windows':.<40} {len(X_tr):,}")
    print(f"  {'Val windows':.<40} {len(X_va):,}")
    print(f"  {'Persistence MAE':.<40} {persist_mae:.4f}  (v22: 0.0346)")
    print(f"  {'LightGBM val MAE':.<40} {mae(y_va, lgb_val):.4f}  (v22: 0.2316, v19: 0.2559)")
    print(f"  {'XGBoost val MAE':.<40} {mae(y_va, xgb_val):.4f}")
    if cat_m:
        print(f"  {'CatBoost val MAE':.<40} {mae(y_va, pred_num(cat_m, X_va, FEATURES)):.4f}")
    print(f"  {'Blend val MAE':.<40} {best_val_mae:.4f}  (v22: 0.2342, v19: 0.2522)")
    print(f"  {'Blend weights':.<40} {' '.join(f'{k}={v:.2f}' for k,v in best_w.items())}")
    print(f"  {'LGB iters (wk1-5)':.<40} {lgb_iters}")
    print(f"  {'-'*56}")
    print(f"  RECENT_YEARS sweep (last-window val, same schema):")
    print(f"  {'  5y  (v22)':.<40} 0.8132")
    print(f"  {'  7y  (this run)':.<40} submit to find out")
    print(f"  {'  8y  (v22_recent8, NB3)':.<40} running")
    print(f"  {'  15y (v19)':.<40} 0.8193")
