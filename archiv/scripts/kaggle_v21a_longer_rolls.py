"""
kaggle_v21a_longer_rolls.py  --  Extended Rolling Windows
==========================================================
Base:        v19 (last-window val, all regions, no score_lag)
Kaggle best: v19 = 0.1930

SINGLE CHANGE vs v19:
  ROLL_WINS = [7, 14, 30, 60, 90, 180, 270, 365]   (was [7,14,30,60,90,180])
  -> adds 270-day and 365-day rolling stats for prec/humidity/tmp/wind
  -> features: 157  (was 133, +24 rolling features)

Hypothesis: prec_roll180_mean was #1 feature (16.2%) in v19.
  Drought operates on annual cycles -- a 365-day window should capture
  this better and further reduce val/Kaggle MAE.
...
Output: /kaggle/working/submission_v21a_longer_rolls.csv
"""
...
# New cache names: features changed, cannot reuse v19 cache
WEEKLY_CACHE  = WORK_DIR / "cache_weekly_v21a.npz"
WINDOWS_CACHE = WORK_DIR / "cache_windows_v21a.npz"
OUT_PATH      = WORK_DIR / "submission_v21a_longer_rolls.csv"
...
# *** CHANGE vs v19: added 270 and 365 ***
ROLL_WINS = [7, 14, 30, 60, 90, 180, 270, 365]
...
def print_importance(lgb_models, features):
    ...
    # Show new 270/365 day features separately
    r270_imp = imp[[i for i,f in enumerate(feat) if "roll270" in f]].sum()
    r365_imp = imp[[i for i,f in enumerate(feat) if "roll365" in f]].sum()
    print(f"\n  Groups:")
    print(f"    Weather (direct):   {100*w_imp/total:>5.1f}%")
    print(f"    Rolling stats:      {100*r_imp/total:>5.1f}%")
    print(f"      of which roll270: {100*r270_imp/total:>5.1f}%  (NEW)")
    print(f"      of which roll365: {100*r365_imp/total:>5.1f}%  (NEW)")
...
def main():
    ...
    n_roll_new = len(ROLL_COLS) * 2 * 3   # 2 new windows (270,365) x 3 stats
    print("=" * 60)
    print(f"  kaggle_v21a_longer_rolls  |  Features: {len(FEATURES)}  |  no score_lag")
    print(f"  CHANGE vs v19: ROLL_WINS extended to include 270 and 365 days")
    print(f"  New rolling features: +{n_roll_new}  ({len(FEATURES)} total, was 133)")
    ...
    print()
    print("=" * 60)
    print(f"  RESULTS -- kaggle_v21a_longer_rolls")
    print(f"  Change vs v19:  ROLL_WINS + 270d + 365d (+{n_roll_new} features)")
    print(f"  {'Features':.<35} {len(FEATURES)}  (v19: 133)")
