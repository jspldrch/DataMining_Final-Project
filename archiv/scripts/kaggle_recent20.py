"""
kaggle_recent20.py  --  Recent-Years Training (Kaggle version)
==============================================================
Base:        run_recent20_local.py (local script)
Val scheme:  20% region holdout (same as local)
Change:      Trains only on last RECENT_YEARS=5 years per region.
             Feature engineering still uses full history (rolling stats need it).
             Only sliding windows are filtered to recent years.

Question:    Does focusing on recent climate patterns improve Kaggle score?
             v19 trained on all 15 years -> Kaggle 0.1930
             This script uses last 5 years per region.

No score_lag. No accelerator needed.
...
Output: /kaggle/working/submission_recent20.csv
"""
...
WEEKLY_CACHE  = WORK_DIR / "cache_weekly.npz"           # same features as v19, reusable
WINDOWS_CACHE = WORK_DIR / "cache_windows_recent20.npz" # recent-filtered, separate
OUT_PATH      = WORK_DIR / "submission_recent20.csv"

def _find_npz(name: str) -> Path:
    # Notebook 3 dataset slugs
    for slug in ["traindataset", "testdataset", "datafiles", "trainthis", "testset",
                 "drought-data", "train-data", "data"]:
...
RECENT_YEARS    = 5     # only sliding windows from last 5 years per region
                        # feature engineering still uses full history

# Ordinal calendar: year*372 + month*31 + day  -> 1 year = 372 ordinal units
...
# ── Weekly feature cache (shared with v19 if present) ────────────────────────
def load_weekly(t0):
...
# ── Recent filter: last N years per region ────────────────────────────────────
def filter_recent_per_region(weekly: pd.DataFrame, recent_years: int) -> pd.DataFrame:
    """
    Per-region cutoff (NOT global): each region keeps only its own last N years.
    Global cutoff would wrongly exclude entire regions that cover earlier time periods
    (each region has a unique 15-year time window, not shared calendar).
    """
    parts = []
...
# ── Sliding windows (with region holdout) ─────────────────────────────────────
def build_windows_holdout(weekly_recent, features, rng):
    """
    Val: 20% of regions, last window each.
    Train: 80% of regions, all recent windows.
    All: all regions, all recent windows (for final training).
    """
    all_regions = sorted(weekly_recent["region_id"].unique())
    n_val = max(1, int(len(all_regions) * VAL_REGION_FRAC))
    ...
    for region, g in weekly_recent.groupby("region_id", sort=False):
        ...
        Xal.append(Xn[idx]); yal.append(yr[idx]); ral.extend([region]*len(idx))

        if region in val_set:
            # Val: last window only
            Xva.append(Xn[nw-1]); yva.append(yr[nw-1]); rva.append(region)
        else:
            # Train: all windows
            Xtr.append(Xn[idx]); ytr.append(yr[idx]); rtr.extend([region]*len(idx))

    def _df(arrs, cols, regions):
        X = pd.DataFrame(np.vstack(arrs).astype(np.float32), columns=cols)
        X["region_id"] = pd.Categorical(regions)
        return X

    X_tr  = _df(Xtr, features, rtr);  y_tr  = np.vstack(ytr).astype(np.float32)
    X_va  = _df([x.reshape(1,-1) for x in Xva], features, rva)
    y_va  = np.vstack(yva).astype(np.float32)
    X_all = _df(Xal, features, ral);  y_all = np.vstack(yal).astype(np.float32)
    return X_tr, y_tr, X_va, y_va, X_all, y_all, val_set

def load_or_build_windows(weekly_recent, features, rng, t0):
    if WINDOWS_CACHE.exists():
        ...
        if list(ck["feature_names"]) == features:
            ...
            def _r(p):
                ...
                return X, ck[f"y_{p}"]
            val_set = set(ck["val_regions"].astype(str))
            return *_r("tr"), *_r("va"), *_r("all"), val_set
        print("  Windows cache outdated -- rebuilding ...")

    print(f"  Building windows (recent {RECENT_YEARS}y, 20% holdout) ... [{elapsed(t0)}]")
    X_tr, y_tr, X_va, y_va, X_all, y_all, val_set = build_windows_holdout(weekly_recent, features, rng)
    np.savez_compressed(WINDOWS_CACHE,
        ...
        val_regions=np.array(sorted(val_set), dtype=object),
...
def main():
    ...
    print("=" * 60)
    print(f"  kaggle_recent20  |  Features: {len(FEATURES)}  |  no score_lag")
    print(f"  RECENT_YEARS={RECENT_YEARS}  |  Val: 20% region holdout")
    print(f"  Question: does limiting to recent {RECENT_YEARS} years improve Kaggle?")
    print(f"  Reference: v19 (all 15 years) -> Kaggle 0.1930")
    print("=" * 60)

    # 1. Weekly features (full history for rolling stats)
    ...
    print(f"  {n_weeks_total:,} rows, {n_regions} regions (full history)")
    for f in FEATURES:
        if f not in weekly.columns: weekly[f] = np.float32(0)

    # 2. Recent filter: per-region cutoff (not global)
    print(f"\n[2/5] Recent filter: last {RECENT_YEARS} years per region ... [{elapsed(t0)}]")
    weekly_recent = filter_recent_per_region(weekly, RECENT_YEARS)
    n_recent = len(weekly_recent)
    pct = 100 * n_recent / n_weeks_total
    print(f"  Full history:   {n_weeks_total:,} weekly rows")
    print(f"  After filter:   {n_recent:,} weekly rows ({pct:.0f}% retained)")
    print(f"  Avg per region: {n_recent / n_regions:.0f} weeks  (full: {n_weeks_total/n_regions:.0f})")
    print(f"  Note: feature values (rolling stats) still use full history")

    # 3. Sliding windows
    print(f"\n[3/5] Sliding windows (20% region holdout) ... [{elapsed(t0)}]")
    X_tr, y_tr, X_va, y_va, X_all, y_all, val_set = load_or_build_windows(
        weekly_recent, FEATURES, rng, t0)
    print(f"  Train: {len(X_tr):,}  Val: {len(X_va):,}  All: {len(X_all):,}")
    print(f"  Val regions: {len(val_set)}")

    last_score = weekly_recent.sort_values("ordinal").groupby("region_id")["score"].last()
    val_regions_list = X_va["region_id"].astype(str).tolist()
    persist = np.column_stack([last_score.reindex(val_regions_list).fillna(0).to_numpy()] * 5)
    persist_mae = mae(y_va, persist)
    show("Persistence baseline (last score repeated)", y_va, persist)
    print(f"  v19 persistence was 0.0346  (different val scheme)")
    ...
    # 5. Final training (all recent windows)
    ...
    print()
    print("=" * 60)
    print(f"  RESULTS -- kaggle_recent20 (last {RECENT_YEARS} years per region)")
    print(f"  {'RECENT_YEARS':.<35} {RECENT_YEARS}  (full history: ~15)")
    print(f"  {'Weekly rows (recent)':.<35} {n_recent:,}  ({pct:.0f}% of full)")
    print(f"  {'Val scheme':.<35} 20% region holdout")
    print(f"  {'Val regions':.<35} {len(val_set)}")
