"""
run_v9.py  –  Drought Severity Prediction v9

Base: run_v7 (MAE 0.8303). run_v8 was worse (0.8423) because:
  - min_child_samples=100 caused underfitting (v7 used 60)
  - air_dryness / compound_stress / lag28 added noise, not signal

v9 additions (v7 features kept exactly):
  1. regional_monthly_mean_score  — per (region, month) average drought score
                                    from training data. Captures "July is always dry in region X".
                                    Safe: static lookup, no distribution shift unlike score_persist.
  2. Learning rate 0.04 → 0.03   — finer gradient steps, early stopping finds better solution
  3. N_ESTIMATORS 1000 → 1500    — more budget; early stopping picks the right count
  4. XGB max_depth 6 → 7         — slightly more capacity for XGB
  5. Blend grid 0.05 steps        — finer weight search (from v8, safe)
  6. min_child_samples stays 60   — reverting v8's over-regularisation

Usage:
    python scripts/run_v9.py
Output: outputs/submission_v9.csv
"""
...
OUT_PATH   = OUT_DIR / "submission_v9.csv"
...
N_ESTIMATORS  = 1500 if not QUICK_MODE else 400   # 1000 → 1500, early stopping picks optimal
...
LGB_PARAMS = dict(
    ...
    learning_rate=0.03,             # 0.04 → 0.03: finer steps, better final MAE
    num_leaves=127,
    min_child_samples=60,           # back to v7 (v8's 100 caused underfitting)
    subsample=0.8,
...
XGB_PARAMS = dict(
    ...
    max_depth=7,                    # 6 → 7: slightly more capacity
    min_child_weight=50,
    subsample=0.8,
    colsample_bytree=0.8,
    reg_alpha=0.1,
...
def build_feature_list() -> list[str]:
    ...
    # regional_mean_score:         overall avg drought level per region (v7)
    # regional_monthly_mean_score: avg drought level per (region, month) — new in v9
    #   captures "July is always a drought month in region X" without distribution shift
    extra = ["regional_mean_score", "regional_monthly_mean_score"]
...
# ─── Regional score statistics ────────────────────────────────────────────────

def compute_region_score_stats(train_raw: pd.DataFrame) -> tuple[pd.Series, pd.DataFrame]:
    """
    Returns:
      region_means         – mean score per region_id
      region_monthly_means – mean score per (region_id, month)
    Both computed from training data only → no leakage.
    """
    region_means = train_raw.groupby("region_id")["score"].mean()
    rmm = (
        train_raw.groupby(["region_id", "month"])["score"]
        .mean()
        .reset_index()
        .rename(columns={"score": "regional_monthly_mean_score"})
    )
    return region_means, rmm
...
def add_score_stats(
    ...
    merged = df[["region_id", "month"]].merge(rmm, on=["region_id", "month"], how="left")
    df["regional_monthly_mean_score"] = merged["regional_monthly_mean_score"].values.astype(np.float32)
    return df
...
def train_lgb_models(
    ...
    for week in range(5):
        ...
        if X_va is not None:
            ...
            fit_kw["callbacks"] = [lgb.early_stopping(60, verbose=False)]
...
def train_cat_models(
    ...
    for week in range(5):
        ...
        if X_va_n is not None:
            fit_kw["eval_set"] = (X_va_n, y_va[:, week].ravel())
            fit_kw["early_stopping_rounds"] = 60
...
def main() -> None:
    ...
    t0 = time.time()
    print("=" * 66)
    print("  Natural Disaster Severity Prediction  -  run_v9.py")
    mode_label = "QUICK (~20 min)" if QUICK_MODE else "FULL (~100 min)"
    ...
    print(f"  New vs v7: +regional_monthly_mean_score, lr=0.03, N_EST=1500, XGB depth=7")
    print("=" * 66)
    ...
    region_means, rmm = compute_region_score_stats(train_raw)
    ...
    train_feat = add_score_stats(train_feat, region_means, rmm)
    test_feat  = add_score_stats(test_feat,  region_means, rmm)
