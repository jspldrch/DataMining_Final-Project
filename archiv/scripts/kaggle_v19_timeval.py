"""
kaggle_v19_timeval.py  --  Last-window validation for all regions
=================================================================
Base: v1 (weather features only, NO score_lag)
Change: val = last window of EVERY region (not 20% holdout)
        -> ~2248 val points instead of 449

No accelerator needed: LGB/XGB/CatBoost run on CPU.

Output: /kaggle/working/submission_v19_timeval.csv
"""
...
WINDOWS_CACHE = WORK_DIR / "cache_windows_v19.npz"
OUT_PATH      = WORK_DIR / "submission_v19_timeval.csv"
...
# ── Knobs ─────────────────────────────────────────────────────────────────────
RANDOM_STATE    = 42
VAL_REGION_FRAC = 0.20   # fallback if last-window val fails
WEEK_BUCKET     = 7
DRY_THRESHOLD   = 1.0
WINDOW_STRIDE   = 1
N_ESTIMATORS    = 1000
...
# ── Weekly feature cache (shared with v1/v2/v3) ───────────────────────────────
def load_weekly(t0):
...
# ── Last-window val for ALL regions ──────────────────────────────────────────
def build_lastwindow_windows(weekly, features):
    """
    Val = last window of EVERY region (2248 points).
    Train = all windows except the last one of each region.

    Since each region covers a unique time period, a global time cutoff
    makes no sense -- this is equivalent and cleaner.
    """
    Xtr, ytr, rtr = [], [], []
    Xva, yva, rva = [], [], []
...
def main():
    ...
    print("=" * 60)
    print(f"  kaggle_v19_timeval  |  Features: {len(FEATURES)}  |  no score_lag")
    ...
    print(f"  (Region-holdout baseline was 0.0321 -- is this higher?)")
    ...
    # 4. Final training
    print(f"\n[4/5] Final training (all regions) ... [{elapsed(t0)}]")
    ...
    print(f"  Persistence MAE:  {mae(y_va, persist):.4f}")
    print(f"  Reference v12:    Kaggle 0.8258")
