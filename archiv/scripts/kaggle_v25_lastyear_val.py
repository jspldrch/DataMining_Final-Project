"""
kaggle_v25_lastyear_val.py  --  Recent 8 Years + Last-Year Validation
======================================================================
Base:        v22_recent8 (RECENT_YEARS=8, no score_lag)
Kaggle best: recent_local_8y = 0.8095  |  v22_5y = 0.8132

SINGLE CHANGE vs v22 (8y):
  Val = last 52 windows per region  (~1 year)  [was: last 1 window]
  -> Val points: ~52 × 2248 = ~116,896  (was 2,248)
  -> Train points: first ~7 years per region  (was ~7.98 years)

Why: with only 1 val point per region (2,248 total), early stopping is
  noisy -- LGB iteration counts vary widely (e.g. 813/633/608/879/836).
  Using the last full year as val (52 windows per region) gives a much
  stronger early stopping signal: ~52× more data points, stable gradient.

Trade-off: ~12% less training data (1 year out of 8 moved to val).
  Final training uses ALL windows (train + val combined) as usual.

NOTE: val MAE here is NOT comparable to v22 (0.2342).
  v22 val = 1 window per region (last time point only)
  v25 val = 52 windows per region (full year, more diverse)
  -> v25 val MAE will be higher by definition.
  -> Compare only Kaggle scores.

Val scheme: last VAL_WEEKS=52 windows per region (~1 year)
Notebook 4 datasets:
  datatrain/train.npz
  datatest/test.npz
  submissionsample/sample_submission.csv

Output: /kaggle/working/submission_v25_lastyear_val.csv
"""
...
# Same 133 features as v19/v22 -> weekly cache is reusable if present
WEEKLY_CACHE  = WORK_DIR / "cache_weekly.npz"
WINDOWS_CACHE = WORK_DIR / "cache_windows_v25.npz"
OUT_PATH      = WORK_DIR / "submission_v25_lastyear_val.csv"

def _find_npz(name: str) -> Path:
    for slug in ["datatrain", "datatest", "traindataset", "testdataset",
                 "datafiles", "trainthis", "testset", "data"]:
...
def _find_sample_sub() -> Path | None:
    for p in ["/kaggle/input/submissionsample/sample_submission.csv",
...
VAL_WEEKS        = 52   # *** CHANGE: last 52 windows per region as val (~1 year) ***
...
# ── Feature list (identical to v19/v22 -- 133 features) ──────────────────────
def build_features():
    f = list(WEATHER_COLS)
...
# ── Feature engineering (identical to v19/v22) ───────────────────────────────
...
# ── Weekly feature cache (same features as v22 -> reusable) ──────────────────
def load_weekly(t0):
...
# ── Val: last VAL_WEEKS windows per region (~1 year) ─────────────────────────
def build_lastyear_windows(weekly_recent, features):
    """
    CHANGE vs v22: val = last VAL_WEEKS windows per region (not just last 1).

    For each region with nw total windows:
      val_start = max(0, nw - VAL_WEEKS)
      Val   = windows [val_start .. nw-1]  -> up to 52 points per region
      Train = windows [0 .. val_start-1]   -> ~7 years per region
      All   = all windows                  -> used for final training
    """
    Xtr, ytr, rtr = [], [], []
    Xva, yva, rva = [], [], []
    Xal, yal, ral = [], [], []
    ...
    for region, g in weekly_recent.groupby("region_id", sort=False):
        ...
        # Val: last VAL_WEEKS windows (or all if region has fewer)
        val_start = max(0, nw - VAL_WEEKS)
        for i in range(val_start, nw):
            Xva.append(Xn[i]); yva.append(yr[i]); rva.append(region)

        # All: every window (for final training)
...
def load_or_build_windows(weekly_recent, features, t0):
    if WINDOWS_CACHE.exists():
        ...
        feat_ok = list(ck["feature_names"]) == features
        vw_ok   = int(ck.get("val_weeks", 0)) == VAL_WEEKS
        if feat_ok and vw_ok:
    ...
    print(f"  Building windows (recent {RECENT_YEARS}y, val={VAL_WEEKS} weeks) ... [{elapsed(t0)}]")
    X_tr, y_tr, X_va, y_va, X_all, y_all = build_lastyear_windows(weekly_recent, features)
...
def main():
    ...
    print("=" * 60)
    print(f"  kaggle_v25_lastyear_val  |  Features: {len(FEATURES)}  |  no score_lag")
    print(f"  CHANGE vs v22(8y): val = last {VAL_WEEKS} windows/region  (was 1)")
    print(f"  Val points: ~{VAL_WEEKS} × 2248 = ~{VAL_WEEKS*2248:,}  (v22: 2,248)")
    print(f"  RECENT_YEARS={RECENT_YEARS}  |  same 133 features as v19/v22")
    print(f"  NB4 slugs: datatrain / datatest / submissionsample")
    print(f"  NOTE: val MAE not comparable to v22 (different val scheme)")
    print("=" * 60)
    ...
    # 3. Sliding windows (last-year val)
    print(f"\n[3/5] Build windows (last {VAL_WEEKS} weeks = val) ... [{elapsed(t0)}]")
    X_tr, y_tr, X_va, y_va, X_all, y_all = load_or_build_windows(weekly_recent, FEATURES, t0)
    val_per_region = len(X_va) / n_regions
    print(f"  Train: {len(X_tr):,}  Val: {len(X_va):,} (~{val_per_region:.0f}/region)  All: {len(X_all):,}")
    print(f"  v22(8y last-win) had: Train ~926k  Val 2,248")
    ...
    show("Persistence baseline (last score per region, repeated)", y_va, persist)
    print(f"  (persistence higher than v22 because val covers the full year, not just last point)")
    ...
    print(f"  v22(8y) LGB iters were: 813/633/608/879/836")
    print(f"  -> more uniform iters here = better calibrated early stopping")
    ...
    lgb_iter_range = max(lgb_iters) - min(lgb_iters)
    print(f"  LGB iter range: {min(lgb_iters)}–{max(lgb_iters)}  (spread={lgb_iter_range})")
    print(f"  v22(8y) iter range was: 608–879  (spread=271)")
    print_importance(lgb_m, FEATURES)

    # 5. Final training (uses ALL windows -- train + val)
    print(f"\n[5/5] Final training (all {len(X_all):,} windows) + submission ... [{elapsed(t0)}]")
    ...
    print()
    print("=" * 60)
    print(f"  RESULTS -- kaggle_v25_lastyear_val")
    print(f"  Change: val = last {VAL_WEEKS} windows/region  (was 1 in v22)")
    print(f"  {'Features':.<40} {len(FEATURES)}  (same as v19/v22)")
    print(f"  {'RECENT_YEARS':.<40} {RECENT_YEARS}")
    print(f"  {'Val windows':.<40} {len(X_va):,}  (~{val_per_region:.0f}/region)")
    print(f"  {'Train windows':.<40} {len(X_tr):,}")
    print(f"  {'All windows (final training)':.<40} {len(X_all):,}")
    print(f"  {'Persistence MAE (val)':.<40} {persist_mae:.4f}")
    print(f"  {'LightGBM val MAE':.<40} {mae(y_va, lgb_val):.4f}  !! not comparable to v22")
    print(f"  {'XGBoost val MAE':.<40} {mae(y_va, xgb_val):.4f}  !! different val scheme")
    if cat_m:
        print(f"  {'CatBoost val MAE':.<40} {mae(y_va, pred_num(cat_m, X_va, FEATURES)):.4f}")
    print(f"  {'Blend val MAE':.<40} {best_val_mae:.4f}")
    print(f"  {'Blend weights':.<40} {' '.join(f'{k}={v:.2f}' for k,v in best_w.items())}")
    print(f"  {'LGB iters (wk1-5)':.<40} {lgb_iters}  spread={lgb_iter_range}")
    print(f"  {'v22(8y) LGB iters':.<40} [813,633,608,879,836]  spread=271")
    print(f"  {'-'*56}")
    print(f"  Known Kaggle scores (to beat):")
    print(f"  {'  recent_local 8y (best)':.<40} 0.8095")
    print(f"  {'  v22 (5y, NB3)':.<40} 0.8132")
    print(f"  {'  v19 (all 15y)':.<40} 0.8193")
