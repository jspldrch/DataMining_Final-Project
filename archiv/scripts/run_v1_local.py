"""
run_v1_local.py  —  v1-Base, lokal, kein Cache
===============================================
Identisch zu kaggle_v1_base.py in der Logik,
aber läuft auf dem Laptop mit CSV-Daten.

Kein Cache: alles wird jedes Mal neu berechnet.
Laufzeit: ~30-60 Min. (Feature Engineering dominiert)

Output: outputs/submission_v1_local.csv
"""
...
OUT_PATH   = OUT_DIR  / "submission_v1_local.csv"
...
def compute_features(train_raw: pd.DataFrame, test_raw: pd.DataFrame, t0: float):
    ...
    # Wöchentliche Repräsentation (letzter Tag pro Woche)
...
def main():
    ...
    print("=" * 60)
    print(f"  run_v1_local  |  Features: {len(FEATURES)}  |  kein score_lag")
    ...
    # 5. Final Training
    print(f"[5a/5] Final Training (alle Regionen) ...  [{elapsed(t0)}]")
