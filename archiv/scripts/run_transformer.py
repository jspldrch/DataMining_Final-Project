...
def build_test_sequences(test_feat: pd.DataFrame, step_features: list) -> tuple[np.ndarray, np.ndarray]:
    """Baut Test-Sequenzen: letzten SEQ_LEN Wochen pro Region."""
    ...
    for region, g in test_weekly.groupby("region_id", sort=False):
        ...
        if len(rows) < SEQ_LEN:
            pad  = np.zeros((SEQ_LEN - len(rows), rows.shape[1]), dtype=np.float32)
...
def train_transformer(X_tr_seq, y_tr, X_va_seq, y_va, n_features: int, seed: int):
    """Trainiert einen Transformer mit Early Stopping. Gibt (model, val_mae) zurück."""
    ...
    criterion  = nn.L1Loss()   # MAE Loss — direkt die Metrik optimieren
...
def main():
    ...
    X_va_raw, y_va = build_sequences(weekly_recent, all_features, skip_regions=tr_regions,  stride=1)
    X_te_raw, test_ids = build_test_sequences(test_feat, all_features)
    ...
    last_scores = (weekly_recent.sort_values("ordinal")
                   .groupby("region_id")["score"].last()
                   .reindex(sorted(val_regions)).fillna(0).to_numpy())
    baseline_pred = np.column_stack([last_scores] * PRED_LEN)
    ...
    for seed in SEEDS:
        ...
        model, best_val_mae = train_transformer(X_tr_sc, y_tr, X_va_sc, y_va, N_FEAT, seed)
 
        val_preds  = predict(model, X_va_sc)
        test_preds = predict(model, X_te_sc)
    ...
    # Ensemble: einfacher Mittelwert über Seeds
    ...
    # Pro Woche aufschlüsseln
    for wk in range(PRED_LEN):
        wk_mae = mae_np(y_va[:, wk], final_val_preds[:, wk])
        print(f"    Woche {wk+1}: MAE = {wk_mae:.4f}")
 
    # ── Analyse (letztes Modell = letzter Seed) ───────────────────────────────
    print(f"\n{'─'*66}")
    print("  ANALYSE")
    print(f"{'─'*66}")
 
    # Letztes trainiertes Modell fuer Analyse wiederherstellen
    # (Final-Modell auf allen Daten, letzter Seed)
    analysis_model = DroughtTransformer(N_FEAT).to(DEVICE)
    torch.manual_seed(SEEDS[-1])
    # Kurz nochmal trainieren nur fuer Analyse (fruehes Stoppen nach wenigen Epochs)
    # -> effizienter: val-Modell vom letzten Seed direkt nutzen
    # Wir trainieren nochmal kurz auf tr-Daten um ein analyse-faehiges Modell zu haben
    _tmp_model, _ = train_transformer(X_tr_sc, y_tr, X_va_sc, y_va, N_FEAT, seed=SEEDS[-1])
 
    feat_imp_df = analyze_feature_importance(
        _tmp_model, X_va_sc, y_va, all_features, n_samples=500
    )
    attn_weights = analyze_attention(_tmp_model, X_va_sc, n_samples=300)
    ...
    # Typ sicherstellen: region_id muss String sein (wie in sample_submission)
    template = pd.read_csv(SAMPLE_SUB)[["region_id"]]
    template["region_id"] = template["region_id"].astype(str)
 
    pred_df = pd.DataFrame({"region_id": test_ids.astype(str)})
    for k in range(PRED_LEN):
        pred_df[f"pred_week{k+1}"] = final_test_preds[:, k]
 
    # left merge: Reihenfolge der sample_submission bleibt erhalten,
    # fehlende Regionen werden mit 0 aufgefuellt
    out = template.merge(pred_df, on="region_id", how="left")
    for col in [f"pred_week{k+1}" for k in range(PRED_LEN)]:
        out[col] = out[col].fillna(0.0)
 
    # Sanity Check
    assert list(out.columns) == ["region_id", "pred_week1", "pred_week2",
                                  "pred_week3", "pred_week4", "pred_week5"], \
        f"Unerwartete Spalten: {list(out.columns)}"
    assert len(out) == len(template), \
        f"Zeilenzahl stimmt nicht: {len(out)} vs template {len(template)}"
    missing = out[[f"pred_week{k+1}" for k in range(PRED_LEN)]].isna().sum().sum()
    if missing > 0:
        print(f"  WARNUNG: {int(missing)} fehlende Predictions -> mit 0 gefuellt")
